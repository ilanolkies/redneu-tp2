...
def regla_sanger(X, W, M, h, lr):
    ...
    z = np.dot( W, y.T*d)
    _x = np.vstack((X[h].T, X[h].T, X[h].T, X[h].T, X[h].T, X[h].T, X[h].T, X[h].T, X[h].T))
    return (_x.T - z) * y
    #z = np.dot(W, np.dot(y.T,d))
    #dw = lr*np.outer(X[h].T - z,y)
...
class Hebbiano:
    ...
    def plot(self):
        ...
        xyz.scatter3D(y[:,0], y[:,1], y[:,2], 'b.', c=y[:,9], s=7, alpha=0.1)
        xyz.plot([0, self.W[0][0]], [0, self.W[0][1]], [0, self.W[0][2]], 'r-')
